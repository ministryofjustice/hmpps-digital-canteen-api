...
class BtPinPhoneClient(
  ...
  fun getPrisonerBalance(btPinPhoneBalanceRequest: BtPinPhoneBalanceRequest): Mono<BtPinPhoneBalanceResponse> =
    getBtToken().flatMap { btAuthResponse ->
      btPinPhoneWebClient
        .post()
        .uri("/pcs/Balance")
  ...
  fun getPrisonerContacts(btPinPhoneControlledNumbersRequest: BtPinPhoneControlledNumbersRequest): Mono<BtPinPhoneControlledNumbersResponse> =
    getBtToken().flatMap { btAuthResponse ->
      btPinPhoneWebClient
        .post()
        ...
        .onErrorMap(WebClientResponseException::class.java) { ex ->
          val error = handleError(ex)
          logger.error("BT contacts request failed for prisoner ${btPinPhoneControlledNumbersRequest.prisonerId}: ${error.userMessage}")
